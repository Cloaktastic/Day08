...
def rerank_cross_encoder(
    ...
    try:
        ...
        for c, emb in zip(candidates, cand_embs):
            ...
            sim = float(np.dot(query_emb, emb) / (q_norm * c_norm)) if q_norm > 0 and c_norm > 0 else 0.0
            item = c.copy()
            item["score"] = sim
            results.append(item)
        results.sort(key=lambda x: x["score"], reverse=True)
...
def rerank(
    ...
    if method == "cross_encoder":
        ...
        # RRF cần nhiều ranked lists - kết hợp semantic và lexical
        from src.task5_semantic_search import semantic_search
        from src.task6_lexical_search import lexical_search as lex_search
        list1 = semantic_search(query, top_k=top_k * 2)
        list2 = lex_search(query, top_k=top_k * 2)
